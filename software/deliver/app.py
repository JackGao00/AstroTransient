"""
AstroTransient GUI - Bilingual | python app.py
"""
import sys, os
...
L = {
    "zh": {
        ...
        "file_label": "上传 CSV 文件",
        "btn_predict": "开始预测", "btn_template": "下载 CSV 模板",
        "plot_label": "光变曲线", "result_label": "结果", "detail_label": "详情",
        "demo_title": "### 随机选取 PLAsTiCC 天体", "btn_random": "随机抽取",
        "pred_label": "预测结果",
        "bench_title": "### 评估模型准确率", "n_samples": "样本数量",
        "btn_bench": "运行测试", "report_label": "报告",
        "footer": "模型: XGBoost | 特征: 47 个 | 类别: 14 | 准确率: ~82%",
        "upload_err": "请上传 CSV 文件", "no_data": "有效数据点不足 (需 >= 5 个)",
        "best_tag": " <-- 最佳", "correct_tag": " <-- 正确!",
        "obs": "观测点", "mjd_range": "MJD 范围", "bands": "波段",
        "top5": "Top-5 预测:", "most_likely": "最可能", "confidence": "置信度",
        "obj_id": "天体 ID", "true_label": "真实标签", "model_pred": "模型预测:",
        "correct": "正确!", "wrong": "错误",
    ...
    "en": {
        ...
        "btn_predict": "Predict", "btn_template": "Download CSV Template",
        "plot_label": "Light Curve", "result_label": "Result", "detail_label": "Details",
        "demo_title": "### Random PLAsTiCC Object", "btn_random": "Random Sample",
        "pred_label": "Predictions",
        "bench_title": "### Evaluate Model Accuracy", "n_samples": "Number of Samples",
        "btn_bench": "Run Benchmark", "report_label": "Report",
        "footer": "Model: XGBoost | Features: 47 | Classes: 14 | Accuracy: ~82%",
        "upload_err": "Please upload a CSV file", "no_data": "Not enough valid data (need >= 5)",
        "best_tag": " <-- BEST", "correct_tag": " <-- CORRECT!",
        "obs": "Observations", "mjd_range": "MJD range", "bands": "Passbands",
        "top5": "Top-5 Predictions:", "most_likely": "Most likely", "confidence": "Confidence",
        "obj_id": "Object ID", "true_label": "True label", "model_pred": "Model predictions:",
        "correct": "CORRECT!", "wrong": "WRONG",
        "bench_tested": "Tested", "bench_acc": "Accuracy", "bench_correct": "Correct", "bench_report": "Per-class Report",
    },
}

# ===== Load model & data (module level - once at startup) =====
...
try:
    ...
    print(f"  Loaded {len(train_df):,} objects")
except Exception as e:
    print(f"  Data not available: {e}")
    print("  Demo and Benchmark tabs will be unavailable.")
    train_df = None
...
def predict_from_csv(file, lang):
    ...
    for name,prob,_ in result:
        bar="#"*int(prob*40); tag=t["best_tag"] if prob==result[0][1] else ""
...
def random_demo_fn(lang):
    t=L[lang]
    if train_df is None:
        msg = "Demo data not installed. Run launcher.py and choose Y to download, or use Upload CSV tab." if lang == "en" else "示例数据未安装。运行 launcher.py 选择 Y 下载，或使用上传 CSV 页面。"
        return None, msg, ""
    ...
    lines=[f"{t['obj_id']}: {row['object_id']}",f"{t['true_label']}: {true_name}","",t["model_pred"]]
...
def batch_test_fn(n_samples, lang):
    t=L[lang]
    if train_df is None:
        return "Demo data not installed. Run launcher.py and choose Y to download." if lang == "en" else "示例数据未安装。运行 launcher.py 选择 Y 下载。"
...
with gr.Blocks(title="AstroTransient") as app:
    ...
    with gr.Tabs():
        with gr.TabItem("Upload CSV / 上传 CSV"):
            inst_md = gr.Markdown(t0["inst"])
            with gr.Row():
                with gr.Column(scale=1):
                    file_input = gr.File(label=t0["file_label"], file_types=[".csv"])
                    with gr.Row():
                        btn_predict = gr.Button("Predict / 开始预测", variant="primary", size="lg")
                        btn_template = gr.DownloadButton("Download Template / 下载模板", value="lightcurve_template.csv",
        ...
        with gr.TabItem("Benchmark / 批量测试"):
...
if __name__ == "__main__":
    ...
    no_browser = args.no_browser or os.environ.get("ASTROTRANSIENT_LAUNCHER") == "1"
    print(f"Starting on http://localhost:{args.port}")
    app.launch(server_name="0.0.0.0", server_port=args.port, share=args.share,
               inbrowser=not no_browser, theme=gr.themes.Soft())
