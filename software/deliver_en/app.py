"""
AstroTransient GUI | python app.py | http://localhost:7860
"""
import sys, os
...
def predict_from_csv(file):
    ...
    if result is None: return None, "Not enough valid data (need >= 5 points).", ""
...
def random_demo_fn():
    if train_df is None:
        return None, "Demo data not installed. Run launcher.py and choose Y to download, or use the Upload CSV tab.", ""
    ...
    lines=[f"Object ID: {row['object_id']}",f"True label: {true_name}","","Model Predictions:"]
...
def batch_test_fn(n_samples):
    if train_df is None:
        return "Demo data not installed. Run launcher.py and choose Y to download."
...
with gr.Blocks(title="AstroTransient") as app:
    ...
    XGBoost-based 14-class light curve classifier. Upload observations and AI identifies the type.
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Demo"):
            gr.Markdown("### Random PLAsTiCC Object")
    ...
    gr.Markdown("---\n*Model: XGBoost (500 trees) | Features: 47 | Classes: 14 | Accuracy: ~82%*")
