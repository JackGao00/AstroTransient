"""
AstroTransient for HuggingFace Spaces deployment.
Auto-downloads PLAsTiCC data on first run.
"""
import sys, os
...
from src.data.download import load_plasticc, download_plasticc, get_lightcurve
...
L = {
    ...
    "en": {
        ...
        "loading_data": "Downloading PLAsTiCC data...",
    },
}
...
def ensure_loaded():
    global model, label_encoder, train_df
...
def predict_from_csv(file, lang):
    if not ensure_loaded(): return None, "Model not loaded", ""
    t=L[lang]
...
def random_demo_fn(lang):
    if not ensure_loaded(): return None, "Data not loaded", ""
    t=L[lang]
...
def batch_test_fn(n_samples, lang):
    if not ensure_loaded(): return "Data not loaded"
    t=L[lang]
...
with gr.Blocks(title="AstroTransient") as app:
    ...
    with gr.Tabs():
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        btn_template = gr.DownloadButton("Download Template / 下载模板",
        ...
        with gr.TabItem("Demo / 随机演示"):
            ...
            btn_demo = gr.Button("Random Sample / 随机抽取", variant="primary", size="lg")
            with gr.Row():
        ...
        with gr.TabItem("Benchmark / 批量测试"):
            ...
            n_slider = gr.Slider(50, 500, value=100, step=50, label="Samples / 样本数")
            btn_bench = gr.Button("Run Benchmark / 运行测试", variant="primary")
